...
Page(withMerchantGuard({
  ...
  async loadPrinterInfo() {
    try {
      const res = await callAdminApi('getPrinterInfo')

      if (res.success && res.data && res.data.printerInfo) {
        this.setData({
          printerInfo: res.data.printerInfo
        })
        // 加载后查询状态
        this.queryStatus()
      } else {
  ...
  async confirmBind() {
    ...
    try {
      ...
      const existingRes = await callAdminApi('getPrinterInfo')
      if (existingRes.success && existingRes.data && existingRes.data.printerInfo) {
        wx.hideLoading()
        wx.showModal({
          title: '提示',
          content: '已绑定打印机，请先解绑',
      ...
      const saveRes = await callAdminApi('savePrinter', {
        printer: printerData
      })

      if (!saveRes.success) {
        throw new Error(saveRes.error || '保存打印机失败')
      }
  ...
  async doUnbindPrinter() {
    try {
      ...
      // 从数据库删除
      const deleteRes = await callAdminApi('deletePrinter', {
        printerId: printerInfo._id
      })

      if (!deleteRes.success) {
        throw new Error(deleteRes.error || '解绑失败')
      }
  ...
  async testPrint() {
    ...
    try {
      ...
      // 查询店铺信息
      const shopRes = await callAdminApi('getShopInfo')
      const shopInfo = shopRes.success ? (shopRes.data?.shopInfo || null) : null
      ...
      // 生成打印内容
      const orderTypeText = testOrder.orderType === 'delivery' ? '外卖' : '订单'
      ...
      if (testOrder.tableNumber) {
        content += `<C><font# bolder=1 height=2 width=2>入口: ${escapeHtml(testOrder.tableNumber)}</font#></C><BR>`
      }
  ...
  async setDensity(e) {
    ...
    try {
      ...
      if (res.result && res.result.success) {
        // 更新数据库
        const updateRes = await callAdminApi('updatePrinterSettings', {
          printerId: printerInfo._id,
          patch: {
            density
          }
        ...
        if (!updateRes.success) {
          throw new Error(updateRes.error || '设置失败')
        }
